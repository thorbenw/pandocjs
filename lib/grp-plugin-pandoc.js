...
exports.github = {
  ...
  getDownloadURL: async (repository, version) => {
    ...
    return `https://github.com/${repository.owner}/${repository.name}/releases/download/${version}/${repository.name}-${version}-${platform}.zip`;
  },
  postProcess: async (repository, version, folder) => {
    ...
    switch (process.platform.toLowerCase()) {
      case 'win32':
        executableFile = 'pandoc.exe';
        break;
      default:
        executableFile = 'pandoc';
        break;
    }
    executableFile = path.join(folder, `${repository.name}-${version}`, executableFile);
    if (!fs.existsSync(executableFile)) {
      throw Error(`Pandoc executale '${executableFile}' could not be found.`);
    } else {
      result.pandoc = executableFile;
